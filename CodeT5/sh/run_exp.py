...
def get_cmd(task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch, warmup,
            model_dir, summary_dir, res_fn, do_train, checkpoint, compress, max_steps=None, save_steps=None, log_steps=None):
    if max_steps is None:
        cmd_str = 'bash CodeT5/sh/exp_with_args.sh %s %s %s %d %d %d %d %d %d %d %d %d %s %s %s %s %s %s' % \
                  (task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch,
                   warmup, model_dir, summary_dir, res_fn, do_train, checkpoint, compress)
    else:
        cmd_str = 'bash CodeT5/sh/exp_with_args.sh %s %s %s %d %d %d %d %d %d %d %d %d %s %s %s %s %s %s %d %d %d' % \
                  (task, sub_task, model_tag, gpu, data_num, bs, lr, source_length, target_length, patience, epoch,
                   warmup, model_dir, summary_dir, res_fn, do_train, checkpoint, compress, max_steps, save_steps, log_steps)
    return cmd_str
...
def run_one_exp(args):
    ...
    cmd_str = get_cmd(task=args.task, sub_task=args.sub_task, model_tag=args.model_tag, gpu=args.gpu,
                      ...
                      do_train=args.do_train, checkpoint=args.checkpoint_path, compress=args.compress)
    print('%s\n' % cmd_str)
    os.system(cmd_str)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--compress", type=str, choices=['8bit', 'deepspeed', 'distillation'])
